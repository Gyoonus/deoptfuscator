...
class HLocalization: public HLocalization{
 public:
  HLocalization(HGraph* graph, const char* name) : HOptimization(graph, name){}

  void Run() OVERRIDE;

  static constexpr const char* kLocalizationPassName = "Localization";

 private:

  DISALLOW_COPY_AND_ASSIGN(HLocalization);
};
