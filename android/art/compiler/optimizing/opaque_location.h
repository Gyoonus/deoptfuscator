 ...
 * implemented in art::HOpaqueLocation, while graph simplifications
 ...
 *
 * This class is named art::HOpaqueLocation to avoid name
 * clashes with the art::ConstantPropagation class defined in
 * compiler/dex/post_opt_passes.h.
 */
class HOpaqueLocation : public HOptimization {
 public:
  HOpaqueLocation(HGraph* graph, const char* name) : HOptimization(graph, name){}
  void Run(uint32_t ref_1, uint32_t ref_2, uint32_t code_off);
  void Run() OVERRIDE;

  static constexpr const char* kOpaqueLocationPassName = "opaque_location";
 ...
 private:
  ...
  uint32_t ref_2_;
  uint32_t code_off_;
  DISALLOW_COPY_AND_ASSIGN(HOpaqueLocation);
};
