...
static u4 LEfrom32(u4 itg){
    return (itg<<24|itg>>24|((itg>>16)<<24)>>16|((itg<<16)>>24)<<16);
}
/*
...
static void dumpCatches(const DexFile* pDexFile, const DexFile::CodeItem* pCode) {
  ...
  const u4 insnsSize = accessor.InsnsSizeInCodeUnits(); 
  // No catch table.
  if (triesSize == 0) {
  ...
  // If tries_size is non-zero and insns_size is odd, insert padding
  if (insnsSize % 2 == 1) {
    //fprintf(gOutFile, "      padding          : 0000\n");
    fprintf(gOutFile, "0000");
  }
  //fprintf(gOutFile, "      catches       : %02x\n", triesSize);
  for (const DexFile::TryItem& try_item : accessor.TryItems()) {
      ...
      //fprintf(gOutFile, "      start_addr       : %08x\n", LEfrom32(start));
      fprintf(gOutFile, "%08x", LEfrom32(start));
      //fprintf(gOutFile, "      insn_count       : %04x\n", try_item.insn_count_>>8|try_item.insn_count_<<8);
      fprintf(gOutFile, "%04x", try_item.insn_count_>>8|try_item.insn_count_<<8);
      //fprintf(gOutFile, "      handler_off      : %04x\n", try_item.handler_off_>>8|try_item.handler_off_<<8);
      fprintf(gOutFile, "%04x", try_item.handler_off_>>8|try_item.handler_off_<<8);
  }
  const uint8_t* handler_data = accessor.GetCatchHandlerData();
//fprintf(gOutFile, "      handlersdata     : %x\n", (int)handler_data);
  if (accessor.TriesSize() == 0 || handler_data == nullptr) {
      ;
  }
  
  else
  {
  const uint8_t* handler_st = handler_data;
//fprintf(gOutFile, "first-byte : %p\n", handler_data);

//fprintf(gOutFile, "  offset   : %p\n", handler_data);
      const uint32_t handlers_size = DecodeUnsignedLeb128(&handler_data);
      
      //fprintf(gOutFile, "    uleb_handler_list_size   : %d\n", handlers_size);

      for (uint32_t i=0; i<handlers_size; ++i){
          int32_t uleb128_count = DecodeSignedLeb128(&handler_data) * 2;
      //    fprintf(gOutFile, "   sleb_handler_size   : %d\n", uleb128_count);
          if (uleb128_count <=0) {
              uleb128_count = -uleb128_count + 1;
          }

      //    fprintf(gOutFile, "   uleb_rest   : ");
          for (int32_t j = 0; j<uleb128_count; ++j){
     //         fprintf(gOutFile, "0x%04x ", DecodeUnsignedLeb128(&handler_data));
      ...
      //fprintf(gOutFile, "   offset-end   : %p\n", handler_data);
      #if 0
      //const uint8_t* tmptmp;
      //tmptmp = EncodeUnsignedLeb128(tmptmp,handlers_size);
      //const uint32_t handlers_size = &handler_data;
      //fprintf(gOutFile, "      uleb_handler_list_size      : %s\n", handler_data);
      fprintf(gOutFile, "      uleb_handler_list_size      : %d\n", handlers_size);
      ...
      for (uint32_t i = 0; i < handlers_size; ++i) {
          ...
          fprintf(gOutFile, "      uleb_rest(ULEB128): \n");
          for (int32_t j = 0; j < uleb128_count; ++j) {
              fprintf(gOutFile, "%04x ", DecodeUnsignedLeb128(&handler_data));
          }
          fprintf(gOutFile, "\n");
      }
      fprintf(gOutFile, "LEBOFFSET_END : %s\n", handler_data);
      #endif
  const uint8_t* handler_ed = handler_data;
  //fprintf(gOutFile, "first-byte : %p\n", handler_data);
  // Handler-pointer test
  //fprintf(gOutFile, "st-addr : %p\n", handler_st);
  //fprintf(gOutFile, "ed-addr : %p\n", handler_ed);
  //fprintf(gOutFile, "diff-addr : %d\n", (int)(handler_ed-handler_st));
  //fprintf(gOutFile, "st-byte : %02x\n", *handler_st);
  //fprintf(gOutFile, "ed-byte : %02x\n", *handler_ed);

  for (int32_t i = 0; i<(int)(handler_ed-handler_st); ++i){
    fprintf(gOutFile, "%02x", *(handler_st+i));
  }
  fprintf(gOutFile, "\n");
}
...
static void dumpCode(const DexFile* pDexFile, u4 idx, u4 flags,
    ...
    //fprintf(gOutFile, "dumpCode()\n");
    //fprintf(gOutFile, "      registers      : %04x\n", accessor.RegistersSize());
    //fprintf(gOutFile, "      registers      : %04x\n",  (accessor.RegistersSize() >> 8) | (accessor.RegistersSize() << 8));
    //fprintf(gOutFile, "CODEITEMS_BYTECODE : %04x ",  (accessor.RegistersSize() >> 8) | (accessor.RegistersSize() << 8));
    //fprintf(gOutFile, "      ins            : %04x\n", (accessor.InsSize()>>8)|(accessor.InsSize()<<8));
    //fprintf(gOutFile, "%04x ", (accessor.InsSize()>>8)|(accessor.InsSize()<<8));
    //fprintf(gOutFile, "      outs           : %04x\n", (accessor.OutsSize()>>8)|(accessor.OutsSize()<<8));
    //fprintf(gOutFile, "%04x ", (accessor.OutsSize()>>8)|(accessor.OutsSize()<<8));
    //fprintf(gOutFile, "      tries          : %04x\n", (accessor.TriesSize()>>8)|(accessor.TriesSize()<<8));
    //fprintf(gOutFile, "%04x ", (accessor.TriesSize()>>8)|(accessor.TriesSize()<<8));
    //fprintf(gOutFile, "      debug_info_off : %08x\n", LEfrom32(accessor.DebugInfoOffset()));
    //fprintf(gOutFile, "%08x ", LEfrom32(accessor.DebugInfoOffset()));
    //fprintf(gOutFile, "      insns_size     : %08x\n",LEfrom32(accessor.InsnsSizeInCodeUnits()));
    //fprintf(gOutFile, "%08x ",LEfrom32(accessor.InsnsSizeInCodeUnits()));
    if (accessor.InsnsSizeInCodeUnits())
    {
      //fprintf(gOutFile, "      insns       :\n");
      ;
    }

    //for (u4 i = 0; i < accessor.InsnsSizeInCodeUnits(); i++){
        //        const u1* bytePtr = (const u1*) &accessor.Insns()[i];
        //const u1* bytePtr = (const u1*) &accessor.Insns()[i];
        //fprintf(gOutFile, "%02x%02x ", bytePtr[0], bytePtr[1]);
    //}
    /*
    for  (u4 i =0; i < accessor.InsnsSizeInCodeUnits(); i++) {
        const u1* bytePtr = (const u1*) &accessor.Insns()[i];
        //fprintf(gOutFile, "%02x%02x ", bytePtr[2*i], bytePtr[2*i+1]);

        fprintf(gOutFile, "%02x%02x ", bytePtr[i], bytePtr[i+1]); 
    }
    */
    if (accessor.InsnsSizeInCodeUnits())
    {
...
static void dumpClass(const DexFile* pDexFile, int idx, char** pLastPackage) {
  ...
  if (!pClassDef.superclass_idx_.IsValid()) {
    ...
    if(strcmp(superclassDescriptor, "Landroid/webkit/WebView;")==0 or strcmp(superclassDescriptor, "Landroid/webkit/WebViewClient;") == 0)
      fprintf(gOutFile, "%s \n", classDescriptor);
  }
  ...
  else if (gOptions.outputFormat == 0xff) {
