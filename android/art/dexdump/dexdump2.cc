...
static void dumpCatches(const DexFile* pDexFile, const DexFile::CodeItem* pCode) {
  ...
  for (const DexFile::TryItem& try_item : accessor.TryItems()) {
    ...
    UNUSED(end);
    fprintf(gOutFile, "      start_addr       : 0x%04x\n", start);
    fprintf(gOutFile, "      insn_count       : 0x%02x\n", try_item.insn_count_);
...
static void dumpCode(const DexFile* pDexFile, u4 idx, u4 flags,
  ...
  fprintf(gOutFile, "      ins_size        : %d\n", accessor.InsSize());
  fprintf(gOutFile, "      outs_size       : %d\n", accessor.OutsSize());
  fprintf(gOutFile, "      tries_size      : %d\n", accessor.TriesSize());
  fprintf(gOutFile, "      debug_info_off  : %d\n", accessor.DebugInfoOffset());
  fprintf(gOutFile, "      insns_size      : %d 16-bit code units\n",
          accessor.InsnsSizeInCodeUnits());
  UNUSED(flags);
  UNUSED(flags);
  const u1* bytePtr = (const u1*) &accessor.Insns()[0];
  fprintf(gOutFile, "      insns           : \n");
  for(u_int32_t i = 0; i < accessor.InsnsSizeInCodeUnits(); i++)  {
  ...
  else
  {
      ...
      fprintf(gOutFile, "      handlers_size       : %d\n", handlers_size);
      // Manually read each handler.
      for (uint32_t i = 0; i < handlers_size; ++i) {
          int32_t uleb128_count = DecodeSignedLeb128(&handler_data) * 2;
          fprintf(gOutFile, "      uleb128_count       : %d\n", uleb128_count);
          if (uleb128_count <= 0) {
              uleb128_count = -uleb128_count + 1;
          }
          fprintf(gOutFile, "      encoded_type_addr_pair    :");
